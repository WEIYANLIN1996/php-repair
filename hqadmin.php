...
if ($_SESSION['admin']=="hqadmin"){
	...
	if ($conn->connect_error) {
		die("连接失败: " . $conn->connect_error);
		 ...
		 //最新信息读取	
		$sql = "SELECT id,name,schoolid,dromnum,number,repairtype,rexq,date FROM repair_data  WHERE result=4";
		...
		$fsql = "SELECT id,name,schoolid,dromnum,number,repairtype,rexq,date FROM repair_data  WHERE result=3";
	...
	<body>
        ...
        <div class="news_info">
        	<table class="table_info" style="width: 100%;">
			   <p style="background: #46A3FF;color: white;">转后勤报修信息(<span style="color:red"><?=$x?></span>)</p>
			  <thead>
			    <tr>
			      <th>姓名</th>
			      <th>学号</th>
			  ...
			  <?php
			    if($fx>0){
						while($row = $fresult->fetch_assoc()){
							...
							echo '<td style="'.'text-align:center;">'.'维修质量:3星,服务态度:3星</td>';
							echo "</tr>";
					    }
			    }
				else{
